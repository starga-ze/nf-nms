...
BootstrapService::BootstrapService(AuthdEventFactory* eventFactory, AuthdActionFactory* actionFactory)
...
std::unique_ptr<AuthdEvent> BootstrapService::schedule(std::chrono::steady_clock::time_point now)
{
    if (!m_eventFactory)
    {
    ...
    case State::Init:
    {
        ...
        return m_eventFactory->create(AuthdEventDomain::Bootstrap,
...
void BootstrapService::handleEvent(AuthdServiceManager& serviceManager, const BootstrapEvent& event)
{
    if (!m_actionFactory)
    {
    ...
    {
        auto action = m_actionFactory->create(AuthdActionDomain::Bootstrap,
...
void BootstrapService::handleAction(AuthdServiceManager& serviceManager, const BootstrapAction& action)
{
...
void BootstrapService::onServerHello(AuthdServiceManager& serviceManager, const pz::ipc::IpcMessage& msg)
{
    (void)msg;
...
{
    std::string name = pz::ipc::IpcProtocol::daemonToStr(pz::ipc::IpcDaemon::Authd);

    auto flag = pz::ipc::IpcProtocol::toFlag(pz::ipc::IpcFlag::Request);

    pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Authd, pz::ipc::IpcDaemon::Ipcd,
...
std::unique_ptr<pz::ipc::IpcMessage> BootstrapService::buildRuntimeReadyMessage() const
{
    ...
    payloadJson["daemon"] = pz::ipc::IpcProtocol::daemonToStr(pz::ipc::IpcDaemon::Authd);
