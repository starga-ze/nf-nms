...
BootstrapService::BootstrapService(CollectordEventFactory* eventFactory, CollectordActionFactory* actionFactory)
...
std::unique_ptr<CollectordEvent> BootstrapService::schedule(std::chrono::steady_clock::time_point now)
{
    if (!m_eventFactory)
    {
    ...
    case State::Init:
    {
        ...
        return m_eventFactory->create(CollectordEventDomain::Bootstrap,
...
void BootstrapService::handleEvent(CollectordServiceManager& serviceManager, const BootstrapEvent& event)
{
    if (!m_actionFactory)
    {
    ...
    {
        auto action = m_actionFactory->create(CollectordActionDomain::Bootstrap,
...
void BootstrapService::handleAction(CollectordServiceManager& serviceManager, const BootstrapAction& action)
{
...
void BootstrapService::onServerHello(CollectordServiceManager& serviceManager, const pz::ipc::IpcMessage& msg)
{
    (void)msg;
...
{
    std::string name = pz::ipc::IpcProtocol::daemonToStr(pz::ipc::IpcDaemon::Collectord);

    auto flag = pz::ipc::IpcProtocol::toFlag(pz::ipc::IpcFlag::Request);

    pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Collectord, pz::ipc::IpcDaemon::Ipcd,
...
std::unique_ptr<pz::ipc::IpcMessage> BootstrapService::buildRuntimeReadyMessage() const
{
    ...
    payloadJson["daemon"] = pz::ipc::IpcProtocol::daemonToStr(pz::ipc::IpcDaemon::Collectord);
